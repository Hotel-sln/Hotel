...
/*	Le but ici est de tester si la connection et la deconnection avec la bdd fonctionnne
    ...
    foreach ($pdo->query("SELECT id, email, role FROM users") as $row){ print_r($row); echo "<br>\n";} 
        Permet d'afficher les resultats de la requette SELECT id, email, role FROM users;.
*/

echo "is pdo set :" . (isset($pdo) ? "true" : "false") . "<br>\n";
$pdo = DBconnect();
echo "is pdo set :" . (isset($pdo) ? "true" : "false") . "<br>\n";
echo "action : SELECT id, email, role FROM users<br>\n";
$pdo->query("SELECT id, email, role FROM users");
echo "is pdo set :" . (isset($pdo) ? "true" : "false") . "<br>\n";
...
/* 
    la page devrait afficher :
        ...
        is pdo set :false
        is pdo set :true
        action : SELECT id, email, role FROM users
        is pdo set :false	
*/
